...
func localResourceResolverTests() -> [TestCase] {
  [
    TestCase("Image scheme decodes URL paths exactly once and ignores URL suffixes") {
      try MainActor.assumeIsolated {
        ...
        for (source, filename) in [
          ("images/%E4%B8%AD%E6%96%87%20%E5%9B%BE.png", "images/中文 图.png"),
          ("images/100%25%23%3F.png", "images/100%#?.png"),
          ("images/literal%2520.png", "images/literal%20.png"),
          ("images/plot.png?v=1#preview", "images/plot.png"),
        ] {
          let url = URL(string: "mdreader-file://document/" + source)!
          try expectEqual(
            handler.resolvedURL(for: url)?.path, root.appendingPathComponent(filename).path)
        }
        for source in ["%2Fetc/passwd", "%2e%2e/secret.png", "/etc/passwd"] {
          try expectEqual(
            handler.resolvedURL(for: URL(string: "mdreader-file://document/" + source)!), nil)
        }
      }
    },
